	...
	[
		{preloadCamera getPosATL (_this select 0)},
		{
			[_this select 0, _this select 1] spawn {
				...
				sleep 2;
				(_this select 0) setPosATL ((_this select 1) getRelPos [3,135]);
				call BIS_fnc_VRFadeIn;
			};
		},
		[_fn_player, _fn_target],
		5,
		{call BIS_fnc_VRFadeOut; (_this select 0) setPosATL ((_this select 1) getRelPos [3,135]); call BIS_fnc_VRFadeIn;}
...
if (alive _sl && vehicle _sl != _sl) then {_moved = _player moveInAny (vehicle _sl)};
if (_moved) exitWith {};
...
0 = {
		...
		if (alive _x && vehicle _x != _x) then {_moved = _player moveInAny (vehicle _x)};
		if (_moved) exitWith {};
} forEach (_squad);