...
if (!isClass (configfile >> "CfgMods" >> "cba")) exitWith {
	["error", "CBA is not installed, cannot read lobby parameters"] call XPT_fnc_log;
	false
};

// Add a CBA event handler
["CBA_beforeSettingsInitialized", {
	// Ensure that logging will work properly
	...
	// Loop through each lobby parameters
	{
		...
		// Iterate through the sub-array to set CBA settings
		{
			...
			["info", format ["Setting CBA setting [%1] to [%2]",_x select 0, _x select 1]] call XPT_fnc_log;
